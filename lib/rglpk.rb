...
module Rglpk
  Glpk_wrapper.constants.each do |c|
    ...
    const_set(c, v) if v.kind_of? Numeric
  end
  ...
  class RowColArray
    ...
    def [](i)
      if i.kind_of?(String)
        fail 'no rows' if self[1].nil?
        idx = Glpk_wrapper.send(glp_find_method, self[1].p.lp, i)
        fail ArgumentError, "no row with name #{i.inspect}" if idx == 0
        @array[idx - 1]
      else
        @array[i]
      end
  ...
  class Problem
    ...
    def del_rows(a)
      ...
      row = Glpk_wrapper.new_intArray(a.size + 1)
      a.each_with_index { |n, i| Glpk_wrapper.intArray_setitem(row, i + 1, n) }
      Glpk_wrapper.glp_del_rows(@lp, a.size, row)
      Glpk_wrapper.delete_intArray(row)
  ...
  class Row
    ...
    def set_bounds(type, lb, ub)
      fail ArgumentError unless TypeConstants.include?(type)
      lb = 0.0 if lb.nil?
      ub = 0.0 if ub.nil?
    ...
    def bounds
      ...
      lb = (t == GLP_FR || t == GLP_UP) ? nil : lb
      ub = (t == GLP_FR || t == GLP_LO) ? nil : ub

      [t, lb, ub]
    end

    def set(v)
      fail RuntimeError unless v.size == @p.cols.size
  ...
  class Column
    ...
    def set(v)
      fail RuntimeError unless v.size == @p.rows.size
      ind = Glpk_wrapper.new_intArray(v.size + 1)
      val = Glpk_wrapper.new_doubleArray(v.size + 1)
  ...
  class ObjectiveFunction
    ...
    def dir=(d)
      fail ArgumentError if d != GLP_MIN && d != GLP_MAX
    ...
    def coefs=(a)
      @p.cols.each do |c|
        Glpk_wrapper.glp_set_obj_coef(@p.lp, c.j, a[c.j - 1])
      end
      a
    end
